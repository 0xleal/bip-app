...
export async function checkRateLimit(
  ...
  try {
    ...
    const dailyCount = dailyUsages?.reduce((sum, record) => sum + record.generation_count, 0) || 0;
    ...
    const weeklyCount = weeklyUsageRecords?.reduce(
    ...
    if (dailyCount >= config.dailyLimit) {
      return {
        allowed: false,
        usage,
        reason: `Daily limit of ${config.dailyLimit} generations exceeded. Resets at ${dailyEnd.toLocaleTimeString()}.`,
      };
    }
    ...
    if (weeklyCount >= config.weeklyLimit) {
      return {
        allowed: false,
        usage,
        reason: `Weekly limit of ${config.weeklyLimit} generations exceeded. Resets in ${Math.ceil((weeklyEnd.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))} days.`,
      };
    }
...
export async function incrementUsage(userId: string): Promise<void> {
  ...
  try {
    ...
    const existingUsage = existingUsages && existingUsages.length > 0 ? existingUsages[0] : null;
